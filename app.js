...
// Revised cookie plugin

(function($){$.cookie=function(key,value,options){if(arguments.length>1){var o=$.extend({},$.cookieOptions,options);if(value===null||value===undefined){value='';o.expires=-1;}
...
(o.secure?'; secure':'');}else{if(result=new RegExp(key+"=(.*?)(?:;|$)").exec(document.cookie))
return decodeURIComponent(result[1]);return false;}};$.cookieOptions={expires:365,path:'/'}})(jQuery);


// Ensure jQuery and Prototype play nice
jQuery.noConflict();
...
AutoSuggest = {
	...
	add: function(tagfield){
		...
		tagfield.keyup(function(e){
			...
			if(tf){ // if not up or down key and or not pagedown or pageup
				...
				wordToMatch = AutoSuggest.latinToLowerCase(wordToMatch);
				...
				if(wordToMatch !== cachedString && tagfield.value !== cachedField && wordToMatch !== ''){
					...
					AutoSuggest.timerId = setTimeout(function(){
			...
			if(e.keyCode === Event.KEY_PAGEDOWN || e.keyCode === Event.KEY_PAGEUP){
				e.preventDefault();
			}
		});
	...
	getWordStartAndEnd: function(inp){
		...
		while(orStart > 0 && (inp.value[orStart - 1] !== ' ' && inp.value[orStart - 1] !== '\n')){
			orStart--;
		}
		rtrn['start'] = orStart;
		rtrn['end'] = rtrn['start'];
    ...
    for (var i = 0; i < 48; i++) {
      noCacheResults += characters.charAt(Math.floor(Math.random() * charactersLength));
    }
		new Ajax.Request('//chan.sankakucomplex.com/tag/autosuggest?tag=' + wordToMatch + '&anti_cache=' + noCacheResults, {
			method: 'get',
	...
	displayMessage: function(message, tagfield){
		...
		AutoSuggest.ul.show().html('<div>' + message + '</div>').css({
			"top": top + "px",
			"left": left + "px",
		});
	},
		...
		if(res[1] && res[0] === wordToMatch){
			if(res[1].length > 0) {
				for(var i = 0; i < res[1].length; i++){
					...
					li.appendChild(new Element('span').update(`${res[1][i]}`).addClassName(`tag-type-${res[4][i]}`));
					if(res[2][i]){
						li.appendChild(new Element('span').update(res[2][i]));
					}
          li.appendChild(new Element('span').update(`${res[3][i] == "e" ? "[R18+]" : res[3][i] == "q" ? "[R15+]" : res[3][i] == "s" ? "[G]" : ""}`).addClassName(`${res[3][i] == "e" ? "autosuggest-tag-explicit" : res[3][i] == "q" ? "autosuggest-tag-questionable" : "autosuggest-tag-safe"}`));
				...
				if(tagfield.tagName === 'TEXTAREA'){
					AutoSuggest.ul.append('<div id="autosuggestPUPD">&#8593; Page Up / Page Down &#8595;</div>');
				}
			}
			else{
				AutoSuggest.ul.hide();
	...
	latinToLowerCase: function(word){
		word = word.split('');
		var c = 0;
		for(var i = 0; i < word.length; i++){
			c = word[i].charCodeAt();
			if(64 < c && c < 91){ // if from A to Z
				word[i] = String.fromCharCode(c + 32); // convert to a to z
			}
		}
		word = word.join('');
		return word;
	...
	vote: function(id, score) {
		new Ajax.Request("/comment/vote.json", {
			...
			onComplete: function(resp) {
				...
				if (resp.success) {
                    $("comment-score-" + resp.id).update(resp.score)
				} else {
					notice("Error: " + resp.reason)
				}
			}
...
var Note = Class.create({
  ...
  showEditBox: function(e) {
    ...
    html += '<div id="edit-box" style="top: '+top+'px; left: '+left+'px; position: absolute; visibility: visible; z-index: 100; background:' + document.body.getStyle("backgroundColor") + '; border: 1px solid black; padding: 12px;">'
...
(function ($) {
	...
	var LazyIFrame = function (el) {
		var url;
		var needFadeInAnimation = false;

		var tagName = el.get(0).tagName.toLowerCase();
		if (tagName == "noscript") {
			var content = el.text();
			content = content.replace(" src=", " data-lazy-iframe-uid=\"" + uidCounter + "\" data-src=");
			el.replaceWith(content);
			el = $("[data-lazy-iframe-uid='" + uidCounter + "']");
			uidCounter++;
		}
	...
	$.fn.lazyIFrame = function () {
		return this.each(function () {
			var el = $(this);
			if (!el.data("lazyIFrame")) {
				var instance = new LazyIFrame(el);
				el.data("lazyIFrame", instance);
			}
			return el.data("lazyIFrame");
		});
	};
})(jQuery);
...
(function() {
    ...
    (function($, exports) {
        exports.sticky_relocate = function() {
    ...
    (function(exports, moduleName) {
        if (typeof exports[moduleName] === "undefined") {
            exports[moduleName] = {};
        }
    })(window, moduleName);
    ...
    (function(exports, prefix) {
        exports.Events = {
            BASE_URL_RECEIVED: prefix + "baseUrlReceived",
            CHANGE_AUTO_ENABLED: prefix + "changeAutoEnabled",
            TOGGLE_AUTO_ENABLED: prefix + "toggleAutoEnabled",
            AUTO_ENABLED_CHANGED: prefix + "autoEnabledChanged",
            PAGE_LOADED: prefix + "pageLoaded"
        };
    })(window[moduleName], moduleName + "#");

    /**
     * Pagination.
     * Handle window "scroll" event, download next pages
     * and perform all related to this actions.
     * This entity is main "engine" so it should be initialized in last order.
     * It use events in order to notify other world about changes.
     *
     * When Pagination "init" method called it is possible to pass options object.
     * Options object can contains:
     *     *) delay {number}
     *     By default: 0
     *     Delay before application start preload first next page
     *     
     *     *) paginationSelector {string}
     *     By default: "div#paginator.auto-page .pagination"
     *     Selector for pagination element.
     *
     *     *) contentContainerSelector {string}
     *     By default: ".content"
     *     Selector for container which will contains loaded pages
     *
     *     *) getPageIDFunction {Function}
     *     By default: function which use /(\d+)$/ RegExp (@see defaultGetPageIDFunction)
     *     Reference to helper function which extract page ID from page data URL
     *
     * When Pagination will be initialized it will perform this actions:
     *     *) find pagination and content container elements
     *     *) if pagination element contains anchor elements then it will try
     *     to extract "base URL" for new anchors (see SideBar). If there is
     *     baseURl then it dispatch Sankaku.Event.BASE_URL_RECEIVED event
     *     *) try extract next pagedata URL from pagination element. It will
     *     looking for value of attribute "next-page-url".
     *     If there is next page data URL and auto loading enabled then
     *     application will load next page data
     *     *) if at startup there are other pages inside content container
     *     element then application notify about this "other world" with help
     *     Sankaku.Event.PAGE_LOADED
     *
     * When page loaded then application perform this actions:
     *     *) try extract next page data URL from loaded page HTML. IT will looking for value
     *     of attribute "next-page-url". If there are no this attribute or value is incorrect
     *     then application will not load next pages.
     *     *) insert new page into content container element
     *     *) start fade in animation for children elements of page
     *     *) notify other world with help Sankaku.Event.PAGE_LOADED event
     *     *) during page data loading and during page children animation loading of
     *     next page data is not occurs. In order to application will able to load
     *     next page data (if required) it wait until it load page data and animation of
     *     last child in this page will be completed.
     */
    ...
    (function($, Events, exports) {
         ...
         * @private
         */
        var COOKIES_EXPIRES = 28;

        /**
         * @type {number}
         * @private
         */
        var FADE_IN_DURATION = 175;

        /**
         * @type {number}
         * @private
         */
        var FADE_IN_DELAY = 250;
        ...
        /**
         * Is next page data loading occurs
         * @type {boolean}
         * @private
         */
        var isLoading = false;
        ...
        /**
         * Is new page items animation occurs
         * @type {boolean}
         * @private
         */
        var isNewPageItemsAnimation = false;
        ...
        /**
         * Delay in seconds when first page preloading will be started.
         * @type {number}
         * @private
         */
        var delay = 0;
        ...
        function parseOptions(options) {
            ...
            contentContainerSelector =
                options[TOKEN_CONTENT_CONTAINER_SELECTOR] || contentContainerSelector;
            getPageIDFunction = options[TOKEN_GET_PAGE_ID_FUNCTION] || getPageIDFunction;

            delay = options[TOKEN_DELAY] || delay;
            if (isNaN(delay)) {
                delay = 0;
            }
            delay *= 1000;
        ...
        /**
         * Notify about existing pages at startup
         * @private
         */
        function notifyAboutExistingPages() {
            var selector = "." + CONTENT_PAGE_CLASS_NAME + ", [" + NEXT_PAGE_URL_ATTRIBUTE_NAME + "]";
            var pages = contentContainer.children(selector);
            ...
            for (var i = 0; i < count; i++) {
                ...
                pageID = pageID.replace(CONTENT_PAGE_ID_PREFIX, "");
                dispatchEvent(Events.PAGE_LOADED, [pageID, page]);
            }
        }

        /**
         * Update auto_enabled value on startup
         * @private
         */
        ...
        //*******************************************
        /**
         * Change auto_enabled value
         * @param {boolean} value
         * @param {boolean} forceChanging
         * @private
         */
        ...
        /**
         * Update on auto_enabled changed
         * @private
         */
        ...
        /**
         * Return auto enabled value from cookies
         * @returns {boolean}
         * @private
         */
        function getAutoEnabledFromCookies() {
            var cookievalue = $.cookie(AUTO_ENABLED_COOKIE_NAME);
            if (cookievalue === '0') {
                return false;
            }
            else 
        ...
        /**
         * Return next page data URL from passed in element
        ...
        function getNextPageDataUrl(element) {
            ...
            var parts = url.split("?");
            if (parts[0].indexOf("post") == -1) {
                parts[0] = parts[0] + "post/";
            }
            if (parts[0].indexOf("index") == -1) {
                parts[0] = parts[0] + "index";
            }
            if (parts[0].indexOf(".content") == -1) {
         ...
         * @private
         */
        function showNewPageItems(children) {
            isNewPageItemsAnimation = true;

            //var children = items;//newPage.children();
        ...
        /**
         * This function will be called when new page data will be loaded
         * and all items inside new page will be shown
         * @private
         */
        ...
        //*******************************************
        /**
         * Dispatch event through document
        ...
        /**
         * Cross-browser scroll check
         * Adapted from: http://www.infinite-scroll.com/infinite-scroll-jquery-plugin/
         * @returns {boolean}
         * @private
         */
        function isNearBottom() {
            ...
            var scrollTop = getScrollTop(); //$(scrollTopOwner).scrollTop();

            // distance remaining in the scroll
            var distance_from_bottom = documentHeight - windowHeight - scrollTop;

            // if distance remaining in the scroll (including buffer) 
            // is less than the original nav to bottom....
        ...
        /**
         * Default implementation of function which extract page ID
         * from URL
         * @param {string} url
         * @returns {string}
         * @private
         ...
         * @private
         */
        function ajax_successHandler(response) {
            isLoading = false;
            processLoadedPageData(response);
        }
        ...
        /**
         * "change auto enabled" event handler
         * @param {Object} event
         * @param {boolean} value
         * @private
         */
        ...
        /**
         * Last child in new page fade in animation complete handler
         * @private
         */
        function lastChild_fadeInCompleteHandler() {
            isNewPageItemsAnimation = false;
        ...
        publicAPI = {
            /**
             * Initialize instance
             * @param {Object} options Options object (see above)
             */
            ...
            /**
             * Is auto loading of pages data enabled.
             * Read only.
             * @type {boolean}
             */
    ...
    })(jQuery, window[moduleName].Events, window[moduleName]);

    /**
     * AdInjector. 
     * It handle Sankaku.Events.PAGE_LOADED events 
     * and inject advertising into loaded pages. 
     */
    ...
    (function($, Events, exports) {
         ...
         * @private
         */
        var TOKEN_FREQUENCY = "frequency";

        /**
         * @type {string}
         * @private
         */
        var TOKEN_AD_HTML_CODE = "advertisingHTML";
         ...
         * @private
         */
        var DEFAULT_AD_HTML_CODE = "<div id='sp1' class='scad-i' align='center'>" +
            "<iframe " +
            "data-src='//c.otaserve.net/gi/300x250x4.html' " +
            "framespacing='0' frameborder='no' scrolling='no' width='1200' height='250'>" +
            "</iframe>" +
            "<div align='left'><a href='https://get.sankaku.plus/' target='_blank'>Hide ads</a></div></div>";
         ...
         * @private
         */
        var adHTMLCode = DEFAULT_AD_HTML_CODE;

        /**
         * Loaded pages counter
         * @type {number}
         * @private
         */
        var pagesCounter = 0;
        ...
        function parseOptions(options) {
            ...
            frequency = options[TOKEN_FREQUENCY] || frequency;
            adHTMLCode = options[TOKEN_AD_HTML_CODE] || adHTMLCode;
        }
         ...
         */
        function addListeners() {
            eventBus.bind(Events.PAGE_LOADED, pageLoadedHandler);
        }
         ...
         * @private
         */
        function processPage(pageElement) {
            pagesCounter++;
            if (pagesCounter < frequency) {
                return;
            }

            pagesCounter = 0;
            insertAd(pageElement);
        }

        /**
         * Insert ad into page element
         * @param {Object} pageElement
         * @private
         */
        function insertAd(pageElement) {
            var ads = $(adHTMLCode).appendTo(pageElement);

            if (typeof $.fn.lazyIFrame === "undefined") {
                return;
            }

            $("iframe", ads).lazyIFrame();
        }
         ...
         * @param {Object} pageElement
         */
        function pageLoadedHandler(event, pageID, pageElement) {
            processPage(pageElement);
        }
        ...
        exports.AdInjector = {
             ...
             * Can contains:
             *     *) frequency {number} By default: 1 
             *     *) advertisingHTML {string} By default: see DEFAULT_AD_HTML_CODE
             */
            init: function(options) {
                initialize(options);
            }
    ...
    /**
     * SideBar.
     * Show information about loaded pages, contains links with usual 
     * URLs.
     * 
     * When SideBar "init" method called it is possible to pass options object.
     * Options object can contains:
     *     *) containerSelector {string}
     *     By default: "#post-list .sidebar"
     *     Selector for element inside which will be added "side bar" element
     *     
     *     *) htmlCode {string}
     *     By default: @see DEFAULT_HTML_CODE
     *     HTML code which will be inserted into container
     *     
     *     *) toggleButtonSelector {string}
     *     By default: "#sc-auto-toggle"
     *     Selector for toggle button.
     *     
     *     *) enabledLabel {string}
     *     By default: "On"
     *     Label of toggle button when auto loading enabled
     *     
     *     *) disabledLabel {string}
     *     By default: "Off"
     *     Label of toggle button when auto loading disabled
     * 
     * In order to get information about changes and about loaded pages 
     * it listen Sankaku.Events.AUTO_ENABLED_CHANGED, Sankaku.Events.PAGE_LOADED and 
     * Sankaku.Events.BASE_URL_RECEIVED events
     */
    ...
    (function($, Events, exports) {
         ...
         * @private
         */
        var TOKEN_HTML_CODE = "htmlCode";

        /**
         * @type {string}
         * @private
         */
        var TOKEN_CONTAINER_SELECTOR = "containerSelector";

        /**
         * @type {string}
         * @private
         */
        var TOKEN_TOGGLE_BUTTON_SELECTOR = "toggleButtonSelector";

        /**
         * @type {string}
         * @private
         */
        var TOKEN_ENABLED_LABEL = "enabledLabel";

        /**
         * @type {string}
         * @private
         */
        var TOKEN_DISABLED_LABEL = "disabledLabel";
         ...
         * @private
         */
        var TOKEN_PAGE_ID = "{PAGE_ID}";

        /**
         * @type {string}
         * @private
         */
        var DEFAULT_HTML_CODE = "<div style='margin-bottom: 1em;'>" +
            "<h5>Auto Paging</h5>" +
            "<ul>" +
            "<li>" +
            "<a href='#' id='sc-auto-toggle'>Enabled: <span data-role='toggle-button-label'>On</span></a>" +
            "</li>" +
            "<li>" +
            "<span id='sc-page-info'></span>" +
            "</li>" +
            "</ul>" +
            "</div>";
         ...
         * @private
         */
        var DEFAULT_CONTAINER_SELECTOR = "#post-list .sidebar";

        /**
         * @type {string}
         * @private
         */
        var DEFAULT_TOGGLE_BUTTON_SELECTOR = "#sc-auto-toggle";

        var DEFAULT_STATUS_CONTAINER_SELECTOR = "#sc-page-info";

        /**
         * @type {string}
         * @private
         */
        var DEFAULT_ENABLED_LABEL = "On";

        /**
         * @type {string}
         * @private
         */
        var DEFAULT_DISABLED_LABEL = "Off";

        /**
         * @type {string}
         * @private
         */
        var DEFAULT_ANCHOR_HTML_CODE = "<a href=\"{URL}\">[{PAGE_ID}]</a>";

        /**
         * @type {string}
         * @private
         */
        var DEFAULT_STATUS_PREFIX = "Pages: ";

        /**
         * @type {number}
         * @private
         */
        var FIRST_PAGE_IDS_COUNT = 3;
         ...
         * @private
         */
        var htmlCode = DEFAULT_HTML_CODE;

        /**
         * @type {string}
         * @private
         */
        var containerSelector = DEFAULT_CONTAINER_SELECTOR;

        /**
         * @type {string}
         * @private
         */
        var toggleButtonSelector = DEFAULT_TOGGLE_BUTTON_SELECTOR;

        /**
         * @type {string}
         * @private
         */
        var toggleButtonLabelSelector = "[data-role='toggle-button-label']";

        /**
         * @type {string}
         * @private
         */
        var statusContainerSelector = DEFAULT_STATUS_CONTAINER_SELECTOR;

        /**
         * @type {string}
         * @private
         */
        var enabledLabel = DEFAULT_ENABLED_LABEL;

        /**
         * @type {string}
         * @private
         */
        var disabledLabel = DEFAULT_DISABLED_LABEL;
        ...
        function parseOptions(options) {
            ...
            htmlCode = options[TOKEN_HTML_CODE] || htmlCode;
            containerSelector = options[TOKEN_CONTAINER_SELECTOR] || containerSelector;
            toggleButtonSelector = options[TOKEN_TOGGLE_BUTTON_SELECTOR] || toggleButtonSelector;
            enabledLabel = options[TOKEN_ENABLED_LABEL] || enabledLabel;
            disabledLabel = options[TOKEN_DISABLED_LABEL] || disabledLabel;
        }
         ...
         */
        function prepareDOM() {
            element = $(htmlCode).appendTo($(containerSelector));
        }
         ...
         */
        function addListeners() {
            $(toggleButtonSelector, element).bind("click", toggleButton_clickHandler);
            eventBus.bind(Events.AUTO_ENABLED_CHANGED, autoEnabledChangedHandler);
            eventBus.bind(Events.PAGE_LOADED, pageLoadedHandler);
            eventBus.bind(Events.BASE_URL_RECEIVED, baseUrlReceivedHandler);
        }
        ...
        /**
         * Update on autoEnabled changed
         * @param {boolean} value
         * @private
         */
        function updateOnAutoEnabledChanged(value) {
            var label = value ? enabledLabel : disabledLabel;
            $(toggleButtonLabelSelector, element).text(label);
        }

        /**
         * Update on page loaded
         * @param {string} pageID
         * @private
         */
        function updateOnPageLoaded(pageID) {
            ...
            if (firstPageIDs.length < FIRST_PAGE_IDS_COUNT) {
            ...
            var anchors = [];
            var count = firstPageIDs.length;
            for (var i = 0; i < count; i++) {
                anchors.push(getAnchorHTMLCode(firstPageIDs[i]));
            }

            var code = "";
            code += DEFAULT_STATUS_PREFIX;
            code += anchors.join(", ");
            ...
            $(statusContainerSelector, element).html(code);
        }

        /**
         * Return anchor HTML code by page id
         * @param {string} pageID
         * @returns {string}
         */
        function getAnchorHTMLCode(pageID) {
            var code = DEFAULT_ANCHOR_HTML_CODE;
            code = code.split(TOKEN_URL).join(getAnchorUrl(pageID));
            code = code.split(TOKEN_PAGE_ID).join(pageID);
            return code;
        }

        /**
         * Return anchor URL by page id
         * @param {string} pageID
         * @returns {string}
         */
        function getAnchorUrl(pageID) {
            return anchorsBaseUrl + pageID;
        }
         ...
         * @private
         */
        function toggleButton_clickHandler(event) {
            event.preventDefault();
            dispatchEvent(Events.TOGGLE_AUTO_ENABLED);
        }

        /**
         * "autoEnabledChanged" event handler
         * @param {Object} event
         * @param {boolean} value
         * @private
         */
        function autoEnabledChangedHandler(event, value) {
            updateOnAutoEnabledChanged(value);
        }
        ...
        function pageLoadedHandler(event, pageID, pageElement) {
            ...
            updateOnPageLoaded(pageID);
        }

        /**
         * "baseUrlReceived" event handler
         * @param {Object} event
         * @param {string} value
         * @private
         */
        function baseUrlReceivedHandler(event, value) {
            anchorsBaseUrl = value;
        }
    ...
    (function($, exports) {
        exports.PostView = {
            init: function() {
                var img_link = $('a#image-link.sample');
                if (!img_link.length) {
                    return;
                }
                ...
                img_link.click(function(event) {
                    if (event.ctrlKey || event.altKey || event.shiftKey) {
                        ...
                        Post.highres();
                        img_link.unbind('click').removeAttr('href');
                        return false;
                    }
                });
            }
        };
    })(jQuery, window[moduleName]);
})();
...
Pool = {
  ...
  remove_post: function(post_id, pool_id) {
	...
	vote: function(id, score) {
		new Ajax.Request("/pool/vote.json", {
			parameters: {
				"id": id,
				"score": score
			},
			onComplete: function(resp) {
				...
				if (resp.success) {
                    $("pool-update-score-" + resp.id).update(resp.score)
				} else {
					notice("Error: " + resp.reason)
				}
			}
...
Post = {
  ...
  blacklist_options: { replace: false },
  ...
  notice_update: function(increase_or_decrease) {
	...
	find_similar: function() {
		...
		$("edit-form").action = "http://sankaku.iqdb.org/"
  ...
  prepare_download: function(URL, fileName) {
  ...
  observe_text_area: function (field_id) {
        ...
        if(typeof field === "undefined" || !field || !field.length) {
                return;
        }
       
        field.observe("keydown", function (e) {
  ...
  vote: function(score, id) {
    ...
    new Ajax.Request("/post/vote.json", {
      ...
      onComplete: function(resp) {
        ...
        if (resp.success) {
          $("post-score-" + resp.post_id).update(resp.score)
          $("post-vote-count-" + resp.post_id).update(resp.vote_count)
        } else {
          notice("Error: " + resp.reason)
        }
      }
  ...
  regenerate_preview: function(id) {
    notice("Regenerating preview for post #"+id)

    new Ajax.Request("/post/regenerate_preview.json", {
      parameters: {
        "id": id,
      },
      ...
      onComplete: function(resp) {
        ...
        if (resp.success) {
          notice("Regenerated preview for post #"+id)
        } else {
          notice("Error: " + resp.reason)
        }
      }
  ...
  resize_image: function() {
    ...
    if ((img.scale_factor == 1) || (img.scale_factor == null)) {
      ...
      var client_width = $("right-col").clientWidth - 240;
...
// Advanced Search
	var newSidebarHeight = 0; // So that the sidebar can all be seen
	var oldSidebarHeight = 0; // So we can switch back
	var oldAdvSearchHeight = 0;
	var a21221 = false;
	...
	var fsize_b = false;
	var gtags_b = false;
	var show = [false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false];
	var opt = ["exc_tags","up_by","fav_by","i_rated","n_rated","source_starts",["id_is","id_is_this"],["h_is","h_is_this"],["w_is","w_is_this"],["sc_is","sc_is_this"],["mp_is","mp_is_this"],["fsize_is","fsize_is_this"],["gtags_is","gtags_is_this"],"i_status","i_parent","sub_of", "order_by", ["d_is", "d_is_this"]]; // I had a point, but I thought of a smarter way to do it...
			function adSearchKeyPressed(e, id1, id2, id3, text){
			if(!e) e=window.event;
			search = document.getElementById("tags");
			if(e.keyCode == 13){
				if(id2 != null && document.getElementById(id2).value != '' && document.getElementById(id2) != null){
					if(id3 != null && document.getElementById(id3).value != '' && document.getElementById(id2).value == 'b' && document.getElementById(id3) != null){
						search.value += " " + text + document.getElementById(id1).value + "..." + document.getElementById(id3).value;
					}else{
						if(document.getElementById(id2).value == '='){
							search.value += " " + text + document.getElementById(id1).value;
						}else{
							search.value += " " + text + document.getElementById(id2).value + document.getElementById(id1).value;
						}
					}
				}else{
					search.value += " " + text + document.getElementById(id1).value;
		...
		function adSearchClick(e, id1, id2, id3, text){
			if(!e) e=window.event;
			search = document.getElementById("tags");
		...
		function add_click(order_type)
		{
		search = document.getElementById("tags");
		var text = 'order:';
		if(search.value == null)
		{
			search.value += text + order_type;	
		}
		else
		{
			search.value += " " + text + order_type;	 
		...
		var text = 'order:';
		var order_type;
		order_type = document.getElementById("order_by").value
		if(search.value == null)
		{
			search.value += text + order_type;	
		}
		...
		j$(document).ready(function(){
			newSidebarHeight = ((j$(window).height() - j$(".sidebar").offset().top - 4) > 50) ? (j$(window).height() - j$(".sidebar").offset().top - 4) : 100;
			oldSidebarHeight = j$(".sidebar").height() - 2;
			oldAdvSearchHeight = j$("#ad_s_e_all").height() - 2;
			j$("#ad_s_e_all span").css('display', 'none');
			j$("#advanced_search_txt").css('display', 'inline');
		});
	...
	function initCloseAdvancedSearch(ab){
		a21221 = ab;
	var onScrFunction = function(e){return false;}
		if(a21221 == true){
			j$("#options").css("display", "block");
			j$("#advanced_search").css("display", "block");
			j$(".sidebar").css('height', '100%');j$(".sidebar").attr('id', dummy);
			j$("#ad_s_e_all").css("overflow", "hidden"); // Scrollbar\
			j$("#ad_s_e_all").css("height", '100%');
			j$("#ad_s_e_all").css('border', 'none');
			j$("#advanced_search_txt").html('<a href="#" onclick="initCloseAdvancedSearch(false);">Hide &raquo;</a>');
			j$(window).bind('scroll', onScrFunction);
		}else{
			j$("#options").css("display", "none");
			j$("#advanced_search").css("display", "none");
			j$(".sidebar").css('height', '100%');
			j$("#ad_s_e_all").css("overflow", "visible");
			j$("#ad_s_e_all").css('border', 'none');
			j$("#ad_s_e_all").css("height", '100%');
			j$("#advanced_search_txt").html('<a href="#" onclick="initCloseAdvancedSearch(true);">Advanced &raquo;</a>');
			j$(window).unbind('scroll', onScrFunction);
		}
		return false;
	}
	...
	function submitAdvancedSearch(){
		var exc = document.getElementById("exc_tags").value;
		var up = document.getElementById("up_by").value;
		var fav = document.getElementById("fav_by").value;
		var irate = document.getElementById("i_rated").value;
		var nrate = document.getElementById("n_rated").value;
		var source = document.getElementById("source_starts").value;
		var id1 = document.getElementById("id_is_this").value;
		var id2 = document.getElementById("id_is_this_2").value;
		var h1 = document.getElementById("h_is_this").value;
		var h2 = document.getElementById("h_is_this_2").value;
		var w1 = document.getElementById("w_is_this").value;
		var w2 = document.getElementById("w_is_this_2").value;
		var sc1 = document.getElementById("sc_is_this").value;
		var sc2 = document.getElementById("sc_is_this_2").value;
		var mp1 = document.getElementById("mp_is_this").value;
		var mp2 = document.getElementById("mp_is_this_2").value;
		var fsize1 = document.getElementById("fsize_is_this").value;
		var fsize2 = document.getElementById("fsize_is_this_2").value;
		var d1 = document.getElementById("d_is_this").value;
		var d2 = document.getElementById("d_is_this_2").value;
		var gtags1 = document.getElementById("gtags_is_this").value;
		var gtags2 = document.getElementById("gtags_is_this_2").value;
		var istatus = document.getElementById("i_status").value;
		var oby = document.getElementById("order_by").value;
		var ipar = document.getElementById("i_parent").value;
		var subo = document.getElementById("sub_of").value;
		var s = "";
		
		if(exc != ""){
			exc.split(" ").join("_"); // turn spaces to underscores
			if(exc.indexOf(",") > -1){
				var temp = exc.split(',');
		...
		var a = [{0:"id_is", 1:"id:", 2:"id1", 3:"id2"}, {0:"h_is", 1:"height:", 2:"h1", 3:"h2"}, {0:"w_is", 1:"width:", 2:"w1", 3:"w2"}, {0:"sc_is", 1:"score:", 2:"sc1", 3:"sc2"}, {0:"mp_is", 1:"mpixels:", 2:"mp1", 3:"mp2"}, {0:"fsize_is", 1:"filesize:", 2:"fsize1", 3:"fsize2"}, {0:"d_is", 1:"date:", 2:"d1", 3:"d2"}, {0:"gtags_is", 1:"general_tag_count:", 2:"gtags1", 3:"gtags2"}];

		for(var i=0;i<8;i++){
			
			if(eval(a[i][2] + " != '' && " + a[i][2] + ".length > 0 && " + a[i][2] + " != 'YYYY-MM-DD'")){
				var temp = a[i][0];
				var temp2 = a[i][1];
				if(eval("document.getElementById(temp).value == 'b' && " + a[i][3] + " != '' ")){
					eval("s += temp2 + " + a[i][2] + " + '...' + " + a[i][3]);
				}else if(document.getElementById(temp).value != '='){
					eval("s += temp2 + document.getElementById(temp).value + " + a[i][2]);
				}else{
					eval("s += temp2 + " + a[i][2]);
				}
				s += " ";
		...
		initCloseAdvancedSearch(false);
	}
	function setTrueFalse(i){
		if(!show[i]){
			if(noOptionsSelected() < 10){
				show[i] = true;
				j$("#al" + i).css('color', '#666');
				if(i >= 6 && i <= 12 || i == 17){
					j$("#" + opt[i][0] + "_s").css('display', 'inline');
				}else{
					j$("#" + opt[i] + "_s").css('display', 'inline');
				}
			}
		}else{
			show[i] = false;
			j$("#al" + i).css('color', '#FF761C');
				if(i >= 6 && i <= 12 || i == 17){
					j$("#" + opt[i][0] + "_s").css('display', 'none');
					//document.getElementById(opt[i][0]).value = '';
					//document.getElementById(opt[i][1]).value = '';
				}else{
					j$("#" + opt[i] + "_s").css('display', 'none');
...
PostModeMenu = {
  ...
  change: function() {
    ...
    var themeModeBG = Cookie.get("theme");
    if (s == "view") {
      document.body.setStyle({backgroundColor: this.original_background_color})
    } else if (s == "edit" && themeModeBG == 0) {
      document.body.setStyle({backgroundColor: "#3A3"})
    } else if (s == "edit" && themeModeBG == 1) {
      document.body.setStyle({backgroundColor: "#006400"})
    } else if (s == "add-fav" && themeModeBG == 0) {
      document.body.setStyle({backgroundColor: "#FFA"})
    } else if (s == "add-fav" && themeModeBG == 1) {
      document.body.setStyle({backgroundColor: "#B29B00"})
    } else if (s == "remove-fav" && themeModeBG == 0) {
      document.body.setStyle({backgroundColor: "#FFA"})
    } else if (s == "remove-fav" && themeModeBG == 1) {
      document.body.setStyle({backgroundColor: "#B29B00"})
    } else if (s == "rating-q" && themeModeBG == 0) {
      document.body.setStyle({backgroundColor: "#AAA"})
    } else if (s == "rating-q" && themeModeBG == 1) {
      document.body.setStyle({backgroundColor: "#191970"})
    } else if (s == "rating-s" && themeModeBG == 0) {
      document.body.setStyle({backgroundColor: "#6F6"})
    } else if (s == "rating-s" && themeModeBG == 1) {
      document.body.setStyle({backgroundColor: "#013220"})
    } else if (s == "rating-e" && themeModeBG == 0) {
      document.body.setStyle({backgroundColor: "#F66"})
    } else if (s == "rating-e" && themeModeBG == 1) {
      document.body.setStyle({backgroundColor: "#8B0000"})
    } else if (s == "approve" && themeModeBG == 0) {
      document.body.setStyle({backgroundColor: "#26A"})
    } else if (s == "approve" && themeModeBG == 1) {
      document.body.setStyle({backgroundColor: "#00003f"})
    } else if (s == "flag" && themeModeBG == 0) {
      document.body.setStyle({backgroundColor: "#FF6347"})
    } else if (s == "flag" && themeModeBG == 1) {
      document.body.setStyle({backgroundColor: "#580000"})
    } else if (s == "regen-preview" && themeModeBG == 0) {
      document.body.setStyle({backgroundColor: "#fc62bf"})
    } else if (s == "regen-preview" && themeModeBG == 1) {
      document.body.setStyle({backgroundColor: "#9c035f"})
    } else if (s == "apply-tag-script" && themeModeBG == 0) {
      document.body.setStyle({backgroundColor: "#A3A"})
    } else if (s == "apply-tag-script" && themeModeBG == 1) {
      document.body.setStyle({backgroundColor: "#800080"})
  ...
  click: function(post_id) {
    ...
    if (s.value == "view") {
      var img = j$('span#p' + post_id + ' a');
      if (Sankaku.Pagination.auto_enabled)
    ...
    } else if (s.value == "regen-preview") {
      Post.regenerate_preview(post_id)
...
PostTagHistory = {
  ...
  update: function() {
    ...
    for (i = 0; i < PostTagHistory.checked.length; ++i) {
      var row = $("r" + PostTagHistory.checked[i].id)
  ...
  undo: function() {
	...
	vote: function(id, score) {
		new Ajax.Request("/post_tag_history/vote.json", {
			parameters: {
				"id": id,
				"score": score
			},
			onComplete: function(resp) {
				...
				if (resp.success) {
                    $("tag-edit-score-" + resp.id).update(resp.score)
				} else {
					notice("Error: " + resp.reason)
				}
			}
...
var Editable = Class.create({
    ...
    setupForm: function() {
        ...
        if (this.editField.tag != 'select') {
            ...
            */
            ...
            this.submitLink = new Element('a', {
                href:'#'
            }).update(Editable.options.saveText);
            if (this.submitButtonClass) this.cancelLink.addClassName(this.submitButtonClass);
        ...
        if (this.editField.type != 'select') {
            ...
            this.editForm.insert(this.submitLink);
    ...
    setupInputElement: function() {
        ...
        this.editField['class'] = 'in-place-edit-field';
    ...
    setupBehaviors: function() {
        ...
        if (this.submitLink) this.submitLink.observe('click', this.save.bindAsEventListener(this));
...
NoticeBox = {
	/*
	 * Stick id="#notice" to the top of the page. Let it  appear and fade out after it's content changes.
	 */
	options: {
		fadeOutAfter: 3000, // start fadeout after this time
		fadeOutTime: 400, // fadeout time, default 400 ms
		fadeOutResponse: 5000, // start fadeout after a page with server response in #notice.
	},
	 
	timer: undefined, // Timer for the fadeOut of the notice-box. If content changes during or before fadeOut, timer needs to be reset.
	
	init: function(){
		/*
		 * Add CSS and ContentChangeListener to #notice.
		 */
		if(jQuery("#content").html().trim().length !== 0){ // If content is not empty
			
			var notice = jQuery("#notice");
			
			// Mutation Observer (instead of DOMSubtreeModified) to check, if the content of the notice-box has changed.
			var observer = new MutationObserver(function(mutations){
				mutations.forEach(function(mutation){
					if(notice.html() !== ""){ // Only mutation if not empty
					
						notice.stop().css({"opacity":"1"});
						
						if(NoticeBox.timer !== undefined){
							clearTimeout(NoticeBox.timer);
						}
						
						NoticeBox.timer = setTimeout(function(){
							notice.fadeOut(NoticeBox.options.fadeOutTime, function(){
								notice.html(""); // Set html to empty, so that next message (if same) can still be registered as mutation
							});
						}, NoticeBox.options.fadeOutAfter);
					}
				});
			});
			var config = {childList:true, subtree:true, characterData:true};
			var target = document.querySelector("#notice");
			observer.observe(target, config);
			
			// Hide notice-box after it is visible, when page is loaded (e.g. after "Response posted.")
			if( notice.is(':visible') ) {
				setTimeout(function(){
					notice.fadeOut();
				}, NoticeBox.options.fadeOutResponse);
			}
			
			// Change style at the end, if error happens
			notice.addClass("status-notice");
			notice.addClass("stick");
...
function setLight() {
    document.cookie = "theme=0; expires=Fri, 31 Dec 9999 23:59:59 GMT; path=/;";
    window.location.reload(true); 
}
function setDark() {
    document.cookie = "theme=1; expires=Fri, 31 Dec 9999 23:59:59 GMT; path=/;";
    window.location.reload(true); 
}

function hideCarousel(action) {
    document.cookie = `hide_carousel=${action === 'hide' ? 1 : 0}; expires=${new Date(Date.now() + 86400000)}; path=/;`;
    window.location.reload(true);
}

function renderDTEXT(areaId, dtext) {

  var input_elem = document.getElementById(areaId);

  var text = input_elem.value;
  var a = input_elem.selectionStart;
  var b = input_elem.selectionEnd;
  var selection = text.substring(a, b);
  ...
  if (dtext === 'URL') {
      ...
      var ext_a = Math.max(0, a - open.length);
      var ext_b = b + close.length;
      var ext_selection = text.substring(ext_a, ext_b);
      ...
      if (ext_selection.startsWith(open) && ext_selection.endsWith(close)) {
          ...
          var open_i = selection.indexOf(open);
          var close_i = selection.lastIndexOf(close);
  ...
  var sel_a = a + sel_off_a;
  var sel_b = sel_a + insert.length + sel_off_b;

  input_elem.value = text.substring(0, cut_a) + insert + text.substring(cut_b);
  input_elem.setSelectionRange(Math.max(0, sel_a), sel_b);
  input_elem.focus();
  return false;
}

async function detectTheme() {
  if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches && Cookie.get("theme") != "0") {
    document.cookie = "theme=1; expires=Fri, 31 Dec 9999 23:59:59 GMT; path=/;";
  }
}
...
document.onkeydown = channelShortcuts;
const delay = ms => new Promise(res => setTimeout(res, ms));
let cooldownState;

async function channelShortcuts(e) {
  let shortcutCooldown = 1250;

  if (!e) e = event;

  /* If typing something, don't call shortcut */
  if ((e.keyCode != '9' && document.activeElement.type !== undefined) || cooldownState === true || e.ctrlKey || e.altKey || e.metaKey) {
    return;
  }

  /* Complete first autocomplete option (tab) */
  else if (e.keyCode == '9' && (document.activeElement.id === "tags" || document.activeElement.id === "post_tags")) {
    e.preventDefault();
    document.querySelector('ul#autosuggest > li:first-child')?.click();
    document.querySelector('input#tags')?.focus();
  }

  /* Search notes (Shift + S) */
  else if (e.shiftKey && e.keyCode == '83' && window.location.pathname.includes("note") && !window.location.pathname.includes("search")) {
    window.location = "/note/search";
    cooldownState = true;
    await delay(shortcutCooldown);
    cooldownState = false;
  }

  /* Search comments (Shift + S) */
  else if (e.shiftKey && e.keyCode == '83' && window.location.pathname.includes("comment") && !window.location.pathname.includes("search")) {
    window.location = "/comment/search";
    cooldownState = true;
    await delay(shortcutCooldown);
    cooldownState = false;
  }
  ...
  /* Previous Page (A OR <-) */
  else if (e.keyCode == '37' && !window.location.pathname.includes("show") || e.keyCode == '65' && !window.location.pathname.includes("show")) {
  ...
  /* Next Page (D OR ->) */
  else if (e.keyCode == '39' && !window.location.pathname.includes("show")  || e.keyCode == '68' && !window.location.pathname.includes("show")) {
  ...
  /* User Settings (Shift + E) */
  else if (e.shiftKey && e.keyCode == '69' && window.location.pathname.includes("user") && !window.location.pathname.includes("edit")) {
  ...
  /* Un-Favorite Post (Shift + F) */
  else if (e.shiftKey && e.keyCode == '70' && window.location.pathname.includes("post/show")) {
    if (document.querySelector('#add-to-favs').style.display === 'none') {
      document.querySelector('#remove-from-favs > .favoriteIcon').click();
      cooldownState = true;
      await delay(shortcutCooldown);
      cooldownState = false;
    } else return;
  ...
  /* Favorite Post (F) */
  else if (e.keyCode == '70' && window.location.pathname.includes("post/show")) {
    if (document.querySelector('#remove-from-favs').style.display === 'none') {
  ...
  /* Vote 1 Star (1) */
  else if (e.keyCode == '49' && window.location.pathname.includes("post/show")) {
	  document.querySelector('.r1-unit').click();
    cooldownState = true;
    await delay(shortcutCooldown);
    cooldownState = false;
  }
  ...
  /* Vote 2 Stars (2) */
  else if (e.keyCode == '50' && window.location.pathname.includes("post/show")) {
	  document.querySelector('.r2-unit').click();
    cooldownState = true;
    await delay(shortcutCooldown);
    cooldownState = false;
  }
  ...
  /* Vote 3 Star (3) */
  else if (e.keyCode == '51' && window.location.pathname.includes("post/show")) {
	  document.querySelector('.r3-unit').click();
    cooldownState = true;
    await delay(shortcutCooldown);
    cooldownState = false;
  }
  ...
  /* Vote 4 Stars (4) */
  else if (e.keyCode == '52' && window.location.pathname.includes("post/show")) {
	  document.querySelector('.r4-unit').click();
    cooldownState = true;
    await delay(shortcutCooldown);
    cooldownState = false;
  }
  ...
  /* Vote 5 Stars (5) */
  else if (e.keyCode == '53' && window.location.pathname.includes("post/show")) {
	  document.querySelector('.r5-unit').click();
    cooldownState = true;
    await delay(shortcutCooldown);
    cooldownState = false;
  }
  ...
  /* Random Post (R) */
  else if (e.keyCode == '82' && window.location.href.match(/\.com\/$|(\/\w{2}(\/|$))|post(\/(show|index)|\/$|$)|\?tags/)) {
  ...
  /* Upload Post (Shift + U) */
  else if (e.shiftKey && e.keyCode == '85' && window.location.href.match(/\.com\/$|(\/\w{2}(\/|$))|post(\/(show|index|similar)|\/$|$)|\?tags/)) {
    window.location = "/post/upload";
    cooldownState = true;
    await delay(shortcutCooldown);
    cooldownState = false;
  }
  ...
  /* Flag Post (Shift + Z) */
  else if (e.shiftKey && e.keyCode == "68" && window.location.pathname.includes("post/show")) {
  ...
  /* Delete Post (Shift + D) */
  else if (e.shiftKey && e.keyCode == "90" && window.location.pathname.includes("post/show")) {
	  for (const a of document.querySelectorAll("a")) {
	    if (a.textContent.includes("Flag")) {
        a.click();
        cooldownState = true;
        await delay(shortcutCooldown);
        cooldownState = false;
  ...
  /* Approve Post (Shift + Z) */
  else if (e.shiftKey && e.keyCode == "79" && window.location.pathname.includes("post/show")) {
	  for (const a of document.querySelectorAll("a")) {
	    if (a.textContent.includes("Approve")) {
        a.click();
        cooldownState = true;
        await delay(shortcutCooldown);
        cooldownState = false;
  ...
  /* Post note history (Shift + N) */
  else if (e.shiftKey && e.keyCode == "78" && window.location.pathname.includes("post/show")) {
	  for (const a of document.querySelectorAll("a")) {
	    if (a.textContent.includes("Notes")) {
        a.click();
        cooldownState = true;
        await delay(shortcutCooldown);
        cooldownState = false;
  ...
  /* Add Translation (N) */
  else if (e.keyCode == "78" && window.location.pathname.includes("post/show")) {
	  for (const a of document.querySelectorAll("a")) {
	    if (a.textContent.includes("Add translation")) {
        a.click();
        cooldownState = true;
        await delay(shortcutCooldown);
        cooldownState = false;
	  ...
	  document.querySelector('.note-body').click();
  }

  /* Post tag history (Shift + T) */
  else if (e.shiftKey && e.keyCode == "84" && window.location.pathname.includes("post/show")) {
	  for (const a of document.querySelectorAll("a")) {
	    if (a.textContent === "Tags") {
        a.click();
        cooldownState = true;
        await delay(shortcutCooldown);
        cooldownState = false;
  ...
  /* Focus comment box (Shift + C) */
  else if (e.shiftKey && e.keyCode == '67' && window.location.pathname.includes("post/show")) {
	  for (const a of document.querySelectorAll("textarea")) {
	    if (a.placeholder.includes('Enter comment...')) {
        a.focus();
        cooldownState = true;
        await delay(shortcutCooldown);
        cooldownState = false;
  ...
  /* New article (N) */
  else if (e.keyCode == "78" && window.location.pathname.includes("wiki/")) {
	  for (const a of document.querySelectorAll("a")) {
	    if (a.href.includes("wiki/add")) {
        a.click();
        cooldownState = true;
        await delay(shortcutCooldown);
        cooldownState = false;
  ...
  /* Edit article (E) */
  else if (e.keyCode == "69" && window.location.pathname.includes("wiki/show")) {
	  for (const a of document.querySelectorAll("a")) {
	    if (a.href.includes("wiki/edit")) {
        a.click();
        cooldownState = true;
        await delay(shortcutCooldown);
        cooldownState = false;
  ...
  /* Article history (H) */
  else if (e.keyCode == "72" && window.location.pathname.includes("wiki/show")) {
	  for (const a of document.querySelectorAll("a")) {
	    if (a.href.includes("wiki/history?title")) {
        a.click();
        cooldownState = true;
        await delay(shortcutCooldown);
        cooldownState = false;
  ...
  /* Article posts (P) */
  else if (e.keyCode == "80" && window.location.pathname.includes("wiki/show")) {
	  for (const a of document.querySelectorAll("a")) {
	    if (a.textContent.includes("Posts (")) {
        a.click();
        cooldownState = true;
        await delay(shortcutCooldown);
        cooldownState = false;
  ...
  /* Rename article (R) */
  else if (e.keyCode == "82" && window.location.pathname.includes("wiki/show")) {
	  for (const a of document.querySelectorAll("a")) {
	    if (a.href.includes("wiki/rename")) {
        a.click();
        cooldownState = true;
        await delay(shortcutCooldown);
        cooldownState = false;
  ...
  /* Lock article (L) */
  else if (e.keyCode == "76" && window.location.pathname.includes("wiki/show")) {
	  for (const a of document.querySelectorAll("a")) {
	    if (a.href.includes("wiki/lock")) {
        a.click();
        cooldownState = true;
        await delay(shortcutCooldown);
        cooldownState = false;
  ...
  /* Unlock article (U) */
  else if (e.keyCode == "85" && window.location.pathname.includes("wiki/show")) {
	  for (const a of document.querySelectorAll("a")) {
	    if (a.href.includes("wiki/unlock")) {
        a.click();
        cooldownState = true;
        await delay(shortcutCooldown);
        cooldownState = false;
  ...
  /* Pool posts (P) */
  else if (e.keyCode == "80" && window.location.pathname.includes("pool/show")) {
	  for (const a of document.querySelectorAll("a")) {
	    if (a.href.includes("/?tags=pool")) {
        a.click();
        cooldownState = true;
        await delay(shortcutCooldown);
        cooldownState = false;
  ...
  /* Pool history (H) */
  else if (e.keyCode == "72" && window.location.pathname.includes("pool/show")) {
	  for (const a of document.querySelectorAll("a")) {
	    if (a.href.includes("pool/history")) {
        a.click();
        cooldownState = true;
        await delay(shortcutCooldown);
        cooldownState = false;
  ...
  /* New pool (N) */
  else if (e.keyCode == "78" && window.location.pathname.includes("pool/")) {
    window.location = "https://beta.sankakucomplex.com/book/add";
    cooldownState = true;
    await delay(shortcutCooldown);
    cooldownState = false;
  }

  /* Mark all dmails as read (Shift + R) */
  else if (e.shiftKey && e.keyCode == "82" && window.location.pathname.includes("dmail/inbox")) {
	  for (const a of document.querySelectorAll("a")) {
	    if (a.href.includes("dmail/mark_all_read")) {
        a.click();
        cooldownState = true;
        await delay(shortcutCooldown);
        cooldownState = false;
  ...
  /* Compose message (dmail inbox) (C) */
  else if (e.keyCode == "67" && window.location.pathname.includes("dmail/inbox")) {
	  for (const a of document.querySelectorAll("a")) {
	    if (a.href.includes("dmail/compose")) {
        a.click();
        cooldownState = true;
        await delay(shortcutCooldown);
        cooldownState = false;
  ...
  /* Compose message (user profile) (M) */
  else if (e.keyCode == "77" && window.location.pathname.includes("user/show")) {
	  for (const a of document.querySelectorAll("a")) {
	    if (a.href.includes("dmail/compose?to=")) {
        a.click();
        cooldownState = true;
        await delay(shortcutCooldown);
        cooldownState = false;
...
/**
 * Initialization code.
 * It should be included after other code.
 */

(function ($) {
	$(function () {
		// this part was extracted from previous implementation.
		$(document).bind(Sankaku.Events.AUTO_ENABLED_CHANGED, function (event, value) {
			if (value) {
				$('.scad').hide();
			}
			else {
		...
		// Initialize AdInjector
		Sankaku.AdInjector.init({

		});

		// Initialize SideBar
		Sankaku.SideBar.init({

		});

		// Initialize Pagination
		// It should be initialize in last order
		Sankaku.Pagination.init({
			//delay: 0
		});
		
		Sankaku.PostView.init();

	});
})(jQuery);

// Initialize lazyIFrame for already existing iframes
(function ($) {
	if(typeof $.fn.lazyIFrame === "undefined") {
		return;
	}
